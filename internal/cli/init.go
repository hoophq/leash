...
// are. Claude Code and Codex speak the same {"hooks": {...}} JSON shape, so
// the converge/remove machinery is shared; OpenCode instead loads a plugin
// file fence generates whole (plugin: true), with its own install path.
...
func newInitCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			switch result {
			case hookInstalled:
				fmt.Fprintf(cmd.OutOrStdout(), "Installed the Fence %s in %s\n", what, path)
				fmt.Fprintf(cmd.OutOrStdout(), "Restart %s (or start a new session) to activate Fence.\n", agent.display)
			case hookUpdated:
				fmt.Fprintf(cmd.OutOrStdout(), "Updated the Fence %s in %s\n", updated, path)
...
// fenceBinary resolves what installed hooks should invoke: the absolute path
// of the current binary so the hook works regardless of PATH, falling back to
// a PATH lookup of "fence". Symlinks stay unresolved: package managers point
// a stable symlink (e.g. /opt/homebrew/bin/fence) at a version-pinned target
// that vanishes on upgrade, so resolving it would break the hook at the next
// `brew upgrade`.
func fenceBinary() string {
...
// hookInvocation returns the command string the agent should run.
func hookInvocation(agent hookAgent) string {
	return fmt.Sprintf("%s hook %s", fenceBinary(), agent.name)
}
